...
async def is_subscribed(bot: Bot, chat_id: str | int, user_id: int) -> bool:
    try:
        ...
        # Bot admin emas / chat topilmadi / private
        return False
...
async def send_broadcast(bot: Bot, from_message: types.Message) -> Tuple[int, int]:
    ...
    for uid in users:
        try:
            # userga aynan admin yuborgan xabarni ko'chirib yuboradi (matn/rasm/video/...)
            await bot.copy_message(chat_id=uid, from_chat_id=from_message.chat.id, message_id=from_message.message_id)
            ok += 1
        except Exception:
            fail += 1
        await asyncio.sleep(0.03)  # flood bo'lmasin
    return ok, fail
...
async def cmd_id(message: types.Message):
    # Guruhda ishlaydi: chat_id chiqaradi
...
async def bot_added(event: types.ChatMemberUpdated, bot: Bot):
    # Bot chatga qo'shilganda adminlarga chat id yuborish
...
async def check_join_cb(call: types.CallbackQuery, bot: Bot):
    ...
    if not ok:
        await call.message.edit_text(
            ...
            "Quyidagilarga kiring va qayta tekshiring:",
            reply_markup=kb_join(missing)
        )
        await call.answer()
        return
...
async def admin_search_start(message: types.Message):
    if not is_admin(message.from_user.id):
        return
...
# --- Universal handler: flow + kod qidirish
@dp.message()
async def universal(message: types.Message, bot: Bot):
    uid = message.from_user.id

    # Majburiy obuna tekshirish (startdan keyin ham)
    ok, missing = await check_required(bot, uid)
    if not ok:
        await message.answer(
...
# =======================
# UPTIMEROBOT uchun PING SERVER (bepul hostingda uxlamaslik uchun)
# =======================
